...
void IpEdit::mouseReleaseEvent(QMouseEvent*)
{
    if(isReadOnly())
        return;
    QPoint	point;
    ...
    QString str = text();
    Calculator calc(str, IPKEY, this);
    // 적당한 키보드의 위치를 얻어낸다
...
// x=250,y=260
QPoint IpEdit::getPtKeyboard(Calculator* calc)
{
...
IpEdit2::IpEdit2( QWidget *parent )
 //   : QLineEdit( parent, name )
    : QLineEdit( parent)
{
}
...
void IpEdit2::mouseReleaseEvent(QMouseEvent*)
{
    if(isReadOnly())
        return;
    QPoint	point;
...
// x=250,y=260
QPoint IpEdit2::getPtKeyboard(Calculator* calc)
{
//	QWidget	*d = QApplication::desktop();
    QDesktopWidget	*d = QApplication::desktop();
    int		w = d->width();		// 화면 전체크기 width
